...
long runTest(FILE* fd1, FILE* fd2, FILE* fd3, FILE* fd4, FILE* fd5, FILE* fd6, FILE* fd7, FILE* fd8, FILE* fd9)
{
    ...
    bool bFile7Done = false;
    bool bFile8Done = false;
    bool bFile9Done = false;
    ...
    FLVRealTimeParser realtimeParser7;
    FLVRealTimeParser realtimeParser8;
    FLVRealTimeParser realtimeParser9;
    unsigned char* result = NULL;
    while(!(bFile1Done && bFile2Done && bFile3Done && bFile4Done & bFile5Done & bFile6Done & bFile7Done & bFile8Done & bFile9Done)) {
        ...
        memcpy(buffer+sizeof(metaData), &mask, sizeof(unsigned int));
        ...
        if ( !bFile1Done ) {
            ...
            //unsigned char streamIdSource[] = {0x01, 0x0}; //desktop stream
                ...
                //fix the mask byte
                mask &= 0xfe;
                memcpy(bigBuf+sizeof(metaData), &mask, sizeof(unsigned int));
                bFile1Done = true;
        ...
        if ( !bFile7Done ) {
            //then send the stream heaer of stream 7
            result = realtimeParser7.readData(fd7, &bufLen);
            if ( result ) {
                //then send the stream heaer of stream 7
                unsigned char streamIdSource[] = {0x32, 0x0};//mobile stream
                memcpy(buffer, &streamIdSource, sizeof(streamIdSource));
                memcpy(buffer+sizeof(streamIdSource), &bufLen, sizeof(unsigned int));
                memcpy((char*)buffer+sizeof(streamIdSource)+sizeof(unsigned int), result, bufLen);
                buffer += (sizeof(streamIdSource)+sizeof(unsigned int)+bufLen);
                ...
                memcpy(bigBuf+sizeof(metaData), &mask, sizeof(unsigned int));
                bFile7Done = true;
                fprintf(stderr,"==file 7 done\r\n");
            }
        }

        if ( !bFile8Done ) {
            //then send the stream heaer of stream 8
            result = realtimeParser8.readData(fd8, &bufLen);
            if ( result ) {
                //then send the stream heaer of stream 8
                unsigned char streamIdSource[] = {0x3a, 0x0};//mobile stream
                memcpy(buffer, &streamIdSource, sizeof(streamIdSource));
                memcpy(buffer+sizeof(streamIdSource), &bufLen, sizeof(unsigned int));
                memcpy((char*)buffer+sizeof(streamIdSource)+sizeof(unsigned int), result, bufLen);
                buffer += (sizeof(streamIdSource)+sizeof(unsigned int)+bufLen);
                ...
                memcpy(bigBuf+sizeof(metaData), &mask, sizeof(unsigned int));
                bFile8Done = true;
                fprintf(stderr,"==file 8 done\r\n");
            }
        }

        if ( !bFile9Done ) {
            //then send the stream heaer of stream 9
            result = realtimeParser9.readData(fd9, &bufLen);
            if ( result ) {
                //then send the stream heaer of stream 9
                unsigned char streamIdSource[] = {0x42, 0x0};//mobile stream
                memcpy(buffer, &streamIdSource, sizeof(streamIdSource));
                memcpy(buffer+sizeof(streamIdSource), &bufLen, sizeof(unsigned int));
                memcpy((char*)buffer+sizeof(streamIdSource)+sizeof(unsigned int), result, bufLen);
                buffer += (sizeof(streamIdSource)+sizeof(unsigned int)+bufLen);
...
int main( int argc, char** argv ) {
  if ( argc != 10 ) {
      fprintf(stderr, "usage: %s input_file1 input_file2 input_file3 input_file4 input_file5 input_file6 input_file7 input_file8 input_file9", argv[0]);
      return -1;
  } 
  //////////////////
  FILE *fp1;
  ...
  if (fseek(fp7, FLV_START_POS , SEEK_SET) != 0) {
    /* Handle Error */
  }
  ...
  if (fseek(fp8, FLV_START_POS , SEEK_SET) != 0) {
    /* Handle Error */
  }
  ...
  if (fseek(fp9, FLV_START_POS , SEEK_SET) != 0) {
    /* Handle Error */
  }

  ///////////////
  runTest( fp1, fp2, fp3, fp4, fp5, fp6, fp7, fp8, fp9);
  return 0;
}
